...
As insercoes no inicio custam O(1) nas tres listas. Insercoes e remocoes no fim custam O(n) na lista simples e circular, e O(1) na lista dupla. Busca por nome, remocao por ID, contagem e exibicao custam O(n). Esvaziar a lista custa O(n). A lista circular percorre seus nos ate voltar ao inicio, sem acessar um ponteiro nulo.
