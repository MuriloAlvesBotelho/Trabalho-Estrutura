...
struct NoDupla { Produto produto; NoDupla *anterior; NoDupla *proximo; };
...
static int inserir(ListaDupla *lista, Produto produto, int no_fim)
{
	...
	novo->produto = produto; novo->anterior = NULL; novo->proximo = NULL;
	if (lista->inicio == NULL) lista->inicio = lista->fim = novo;
	else if (no_fim) { novo->anterior = lista->fim; lista->fim->proximo = novo; lista->fim = novo; }
...
int dupla_remover_id(ListaDupla *lista, int id, Produto *removido)
{
	...
	if (!no) return lista->inicio ? LISTA_NAO_ENCONTRADO : LISTA_VAZIA;
