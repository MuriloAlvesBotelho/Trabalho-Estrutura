...
static void menu(const char *titulo, int reverso)
{
    printf("\n===== %s =====\n1. Inserir no inicio\n2. Inserir no fim\n3. Remover do inicio\n4. Remover do fim\n5. Remover por ID\n6. Buscar por substring no nome\n7. Atualizar quantidade por ID\n8. Exibir todos os produtos\n", titulo);
    if (reverso) printf("9. Exibir em ordem reversa\n10. Contar produtos\n11. Esvaziar lista\n0. Voltar\n");
    else printf("9. Contar produtos\n10. Esvaziar lista\n0. Voltar\n");
}

static Produto novo_produto(int (*existe)(int, void *), void *lista)
{
    Produto produto;
    do { produto = produto_ler(); if (existe(produto.id, lista)) printf("Ja existe um produto com esse ID. Informe outro.\n"); } while (existe(produto.id, lista));
    return produto;
}
...
{
    int op, id, qtd, r; char busca[TAM_NOME]; Produto p, removido;
    do {
        menu("PERECIVEIS - LISTA SIMPLES", 0); op = ler_inteiro(">> ");
        switch (op) {
        ...
        case 6: ler_linha("Substring: ", busca, TAM_NOME); if (!simples_buscar_nome(lista, busca)) printf("Nenhum produto encontrado.\n"); break;
...
static void nao_pereciveis(ListaDupla *lista)
{
    int op, id, qtd, r; char busca[TAM_NOME]; Produto p, removido;
    do {
        menu("NAO PERECIVEIS - LISTA DUPLA", 1); op = ler_inteiro(">> ");
        switch (op) {
...
int main(void)
{
    ListaSimples simples; ListaDupla dupla; ListaCircular circular; int op;
    simples_inicializar(&simples); dupla_inicializar(&dupla); circular_inicializar(&circular);
    estoque_simples = &simples; estoque_dupla = &dupla; estoque_circular = &circular;
    do {
